...
Node *insert(Node *T, int v, int r)
{
  ...
  int newTemp = r - temp - 1;
  if(r > temp)
  {
      
      T->right = insert(T->right, v, newTemp);
  }
  if(r < temp)
  {
      
      T->left = insert(T->right, v, r);
  }
  if(r == temp)
  {
      Node *tempNode = new Node(v);
      tempNode->right = T;
      tempNode->left = T->left;
      T = tempNode;
  }
  fix_size(T);
  return T;
...
void split(Node *T, int r, Node **L, Node **R)
{
  ...
  if(r > temp)
  {
      *L = T;
      int newTemp = r - temp - 1;
      split(T->right, newTemp, &T->right, R);
  }
  else
  {
      *R = T;
...
Node *insert_random(Node *T, int v, int r)
{
  ...
  if(rand() % (T->size) ==0){
      tempNode = new Node(v);
      split(T,r, &tempNode->left, &tempNode->right);
      fix_size(tempNode);
      return tempNode;
  }
  ...
  if(r <= temp){
      T->left = insert_random(T->left,v,r);
       ...
       int newest = r - temp- 1;
      T->right = insert_random(T->right, v, newest);
  }
  fix_size(T);
  return T;
}
  ...
  for (int i=1; i<n; i++) 
  {
    ...
    else 
    {
        ...
        while(low + 1 != high)
        {
            ...
            if(did_x_beat_y(select(T, between)->key, i))
            {
                low = between;
            }
            else
